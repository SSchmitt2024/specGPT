...
- **Figure ref expansion (`_expand_referenced_figures`).** After each rerank, scans context text for `Figure N` mentions and fetches any referenced table not already in context. Ranked by reference frequency, capped at 6. Pure dict probes — no LLM call, no embedding. Appended to the tail of the ranked list so they never displace a ranked chunk. Because they sit at the tail, a prose-saturated context budget used to starve them out entirely (the model cited "[Figure N]" it never received); context assembly now rescues them with a separate additive reserve, see Context Assembly.
...
- **Referenced figures get a separate additive reserve (two-pass fill).** Assembly runs two passes over the ranked chunks. Pass 1 fills `max_context_tokens` in rank order (prose-first, generous `// 3` per-table trim). Pass 2 then fits the tail-appended referenced-figure pulls (`figure_ref_expansion` / `agentic_fetch_figure`) into their own `figure_reserve_tokens` budget on top of the prose budget, with a tight ~450-tok per-figure trim so the whole 6-figure batch fits instead of one 25k-tok table evicting the rest. Without the reserve, a prose-saturated Pass 1 exhausts the budget before reaching the tail figures and the model cites "[Figure N]" it never received. The reserve is headroom, so figures never displace a ranked prose hit and prose never starves the figures the answer depends on. A reserved figure only needs to be SEEN and identifiable (header + caption + first rows of a byte-layout table); deep full-table detail is the agentic loop's job. The two passes dedup by chunk id (falling back to object identity), so a figure that already fit in Pass 1 is not re-emitted. `figure_reserve_tokens=0` restores the pre-fix single-pass behaviour.
- **Empty-bodied chunks are dropped.** A chunk that trims to whitespace grounds nothing, invites a citation of an empty figure, and trips the LLM's 400 BadRequest on an empty content block; assembly skips it in both passes.
...
- **Figure reserve scales with preset.** `figure_reserve_tokens` is 1500 for Fast (leaner, to match its flat cost/latency profile) and 3000 for Balanced/Thorough (sized to fit a full 6-figure expansion batch at the ~450-tok per-figure trim). It only consumes tokens when the answer actually references a figure, so the cost estimate bills ~1.5 trimmed figures as typical with the full reserve as the worst case.
